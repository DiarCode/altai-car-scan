...
export class LearnerSessionService {
	...
	async upsertSession(learnerId: number): Promise<string> {
		...
		const existing = await this.prisma.learnerSession.findUnique({
			where: { learnerId },
		})
		const token = this.jwtService.signLearner({ learnerId })

		if (existing) {
			const updated = await this.prisma.learnerSession.update({
		...
		const created = await this.prisma.learnerSession.create({
	...
	async validateLearnerSession(token: string): Promise<number> {
		...
		const session = await this.prisma.learnerSession.findFirst({
	...
	async revokeLearnerSession(token: string): Promise<void> {
		await this.prisma.learnerSession.deleteMany({ where: { token } })
	}
}
